...
export default function RefundModal({ isOpen, onClose, onConfirm, caseData }: RefundModalProps) {
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-lg border border-slate-200 shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-md p-4 space-y-3 font-sans text-xs">
            ...
            <div className="flex justify-between py-1">
              ...
              <span className="font-semibold text-emerald-600 flex items-center gap-1">
                ...
                Eligible (Auto-Refund Policy 4.2)
              </span>
            </div>
          </div>
        ...
        <div className="bg-slate-50 px-6 py-3 border-t border-slate-200 flex justify-end space-x-3">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-white hover:bg-slate-100 text-slate-700 rounded-md text-xs font-semibold border border-slate-300 cursor-pointer transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-md text-xs font-semibold flex items-center space-x-2 shadow-xs cursor-pointer transition-colors"
          >
            {isSubmitting ? (
              <>
