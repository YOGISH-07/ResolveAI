...
import { ShoppingBag, CreditCard, Truck, CheckCircle2, AlertCircle } from 'lucide-react';

export default function OrdersView({ orders = [], payments = [] }) {
  return (
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-2xs overflow-hidden space-y-2">
        ...
        <table className="w-full text-left text-xs border-collapse">
          ...
          <tbody className="divide-y divide-slate-200">
            {orders.map((ord) => (
              <tr key={ord.id} className={`hover:bg-slate-50 ${ord.id === '#RV-4821' ? 'bg-indigo-50/40' : ''}`}>
          ...
          <tbody className="divide-y divide-slate-200">
            {payments.map((tx) => (
              <tr key={tx.id} className={tx.isDuplicate ? 'bg-amber-50/80 font-medium' : 'hover:bg-slate-50'}>
                ...
                <td className="py-3 px-4 font-mono font-bold text-indigo-600">{tx.orderId}</td>
