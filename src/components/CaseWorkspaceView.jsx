...
export default function CaseWorkspaceView({ 
  ...
  // Auto scroll or sequential timeline effect when running investigation
  const handleStartInvestigation = async () => {
    setIsInvestigating(true);
    setVisibleEventsCount(0);
    
    // Animate events arriving sequentially
    const totalEvents = caseData?.timelineEvents?.length || 9;
    ...
    onRunInvestigation(caseData.id);
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-150 pb-16">
      ...
      <div className="bg-slate-900 text-white p-5 rounded-lg border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <div className="flex items-center space-x-3">
              <h1 className="text-lg font-bold text-white tracking-tight">CASE {caseData.id}</h1>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Customer: <span className="font-semibold text-white">{caseData.customerName}</span> • Multi-Issue Autonomous Investigation Workflow
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          {isEscalated && !isResolved && (
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white rounded text-xs font-semibold shadow-xs transition-colors"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-3 space-y-5">
          ...
          <div className="bg-slate-900 text-slate-300 rounded-lg p-4 space-y-3 text-xs border border-slate-800">
            ...
            <ul className="space-y-2 text-[11px] leading-relaxed">
              ...
              <li className="flex items-start gap-1.5">
                <span className="text-amber-400 font-bold">⚠ §7.1</span>
                <span>Physical Damage Replacement requires Human Sign-off</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="lg:col-span-6 space-y-6">
              ...
              {!hasInvestigationRun ? (
                <div className="text-center py-8 bg-slate-50 rounded-lg border border-dashed border-slate-300 space-y-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center space-x-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md text-xs font-bold shadow-sm transition-colors"
                  >
                ...
                <>
                  ...
                  {!isInvestigating && visibleEventsCount < caseData.timelineEvents.length && (
                    <button
                      onClick={() => setVisibleEventsCount(caseData.timelineEvents.length)}
                      className="text-xs font-semibold text-indigo-600 hover:underline pt-1"
                    >
                      Show all timeline logs ({caseData.timelineEvents.length})
                    </button>
                  )}
                </>
              )}
            ...
            <div className="bg-white rounded-lg border border-slate-200 shadow-2xs overflow-hidden animate-in fade-in duration-200">
              ...
              <div className="p-5 space-y-3 text-xs">
                <div className="p-3 bg-slate-50 border border-slate-200 rounded space-y-1">
                  ...
                  <p className="text-slate-600">{caseData.evidence.payment}</p>
                </div>
                <div className="p-3 bg-slate-50 border border-slate-200 rounded space-y-1">
                  <span className="font-bold text-slate-900 block">Delivery Audit:</span>
                  <p className="text-slate-600">{caseData.evidence.delivery}</p>
                </div>
                <div className="p-3 bg-slate-50 border border-slate-200 rounded space-y-1">
                  <span className="font-bold text-slate-900 block">Complaint Verification:</span>
                  <p className="text-slate-600">{caseData.evidence.complaint}</p>
                </div>
                <div className="grid grid-cols-2 gap-3 pt-1">
                  <div className="p-2.5 bg-emerald-50 border border-emerald-200 rounded">
                    ...
                    <p className="text-[11px] text-emerald-700">{caseData.evidence.policyRefund}</p>
                  </div>
                  <div className="p-2.5 bg-amber-50 border border-amber-200 rounded">
                    ...
                    <p className="text-[11px] text-amber-800">{caseData.evidence.policyReplacement}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {hasInvestigationRun && (
            <div className="bg-slate-900 text-white rounded-lg border border-slate-800 shadow-md overflow-hidden animate-in fade-in duration-200">
              ...
              <div className="p-5 space-y-4 text-xs">
                ...
                <div>
                  <span className="text-slate-400 font-medium block mb-1">Intent Extraction:</span>
          ...
          {/* DECISION PANEL — DUAL SAFE ACTIONS */}
          {hasInvestigationRun && (
            <div className="bg-white rounded-lg border-2 border-indigo-600 shadow-md p-6 space-y-5 animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Action 1: Initiate Refund */}
                <div className="p-4 rounded-md border border-slate-200 bg-slate-50 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-xs text-slate-900">1. Duplicate Payment Refund</span>
                    {isRefundDone ? (
                  ...
                  <ul className="text-[11px] text-slate-600 space-y-1 font-mono">
                    <li>✓ Policy eligible (§4.2)</li>
                    <li>✓ Transaction TX-8832 verified</li>
                  </ul>
                  <button
                    onClick={onInitiateRefund}
                    disabled={isRefundDone}
                    className={`w-full py-2 px-3 rounded text-xs font-bold flex items-center justify-center space-x-2 transition-all ${
                      isRefundDone 
                ...
                {/* Action 2: Escalate Replacement */}
                <div className="p-4 rounded-md border border-amber-200 bg-amber-50/60 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-xs text-amber-950">2. Replacement Request</span>
                    {isEscalated ? (
        ...
        <div className="lg:col-span-3 space-y-5">
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-2xs space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="text-[11px] font-mono text-slate-400">Customer since 2024</div>
            </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-2xs space-y-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 pb-2 border-b border-slate-200 flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono text-indigo-600 font-bold">#RV-4821</span>
            </h3>

            <div className="text-xs space-y-2">
              <div>
                <span className="font-bold text-slate-900 block">Wireless Noise-Canceling Headphones</span>
                <span className="text-[11px] text-slate-400 font-mono">SKU: WNC-H800-BLK</span>
              </div>
              ...
              <div className="text-[11px] text-slate-400 pt-1 font-mono">
                Delivered 2026-09-10 via Express Logistics
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2.5 text-xs">
              ...
              <div className="p-2.5 rounded bg-amber-50 border border-amber-200 space-y-0.5">
                ...
                <div className="flex justify-between text-[10px] text-amber-800 font-mono">
                  <span>UPI Payment (HDFC)</span>
                  <span className="font-bold text-amber-700">DUPLICATE</span>
                </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-2xs space-y-3">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between py-1 border-b border-slate-100">
                <div>
                  <span className="font-mono text-indigo-600 font-bold">RV-3120</span>
