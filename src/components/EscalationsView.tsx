...
import { EscalationPackage } from '../types/index.ts';

interface EscalationsViewProps {
  escalations: EscalationPackage[];
  onSelectEscalatedCase: (caseId: string) => void;
}

export default function EscalationsView({ escalations = [], onSelectEscalatedCase }: EscalationsViewProps) {
  return (
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 text-xs text-purple-950 flex items-start space-x-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-purple-900">
            ResolveAI conducts full autonomous investigation before escalation. Only items requiring manager policy sign-off are routed to human agents with 100% compiled context packages.
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-200 text-xs">
              {escalations.map((esc, i) => {
                const isTarget = esc.caseId.includes('4821');
                return (
                  <tr
                    key={i}
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4 text-right whitespace-nowrap">
                      <button className="px-3 py-1 bg-purple-600 hover:bg-purple-700 text-white rounded text-[11px] font-bold inline-flex items-center space-x-1 shadow-2xs cursor-pointer">
